...
class firehose(metaclass=LogBase):
    ...
    def xmlsend(self, data, skipresponse=False) -> response:
        ...
        rdata = bytearray()
        counter = 0
        timeout = 50  # 5s total (50 * 0.1s) for xmlsend response wait
        if not skipresponse:
    ...
    def cmd_nop(self):
        ...
        tmp = None
        empty_count = 0
        while tmp != b"" and empty_count < 10:  # Limit retries for slow devices
            tmp = self.cdc.read(timeout=None)
            if tmp == b"":
                empty_count += 1
                import time
    ...
    def wait_for_data(self):
        tmp = bytearray()
        timeout = 0
        max_timeouts = 50  # Increased from 4 (0.4s) to 50 (5.0s) for UFS devices
    ...
    def cmd_read(self, physical_partition_number, start_sector, num_partition_sectors, filename, display=True):
        ...
        # For Oppo UFS devices, add retry with USB reset on failure
        max_retries = 3
    ...
    def cmd_read_buffer(self, physical_partition_number, start_sector, num_partition_sectors, display=True):
        self.lasterror = b""
        prog = 0
        # Ensure UFS devices always use 4096-byte sectors
        sector_size = self.cfg.SECTOR_SIZE_IN_BYTES
        if self.cfg.MemoryName.lower() == "ufs" and sector_size != 4096:
            sector_size = 4096
        if display:
            self.info(
                f"\nReading from physical partition {str(physical_partition_number)}, " +
        ...
        data = f"<?xml version=\"1.0\" ?><data><read SECTOR_SIZE_IN_BYTES=\"{sector_size}\"" + \
               f" num_partition_sectors=\"{num_partition_sectors}\"" + \
               f" physical_partition_number=\"{physical_partition_number}\"" + \
               f" start_sector=\"{start_sector}\""
        data += self.nand_pages_attr() + "/>\n</data>"
        ...
        progbar = progress(self.cfg.SECTOR_SIZE_IN_BYTES)
        rsp = self.xmlsend(data, self.skipresponse)
        if isinstance(rsp.data, dict) and "value" in rsp.data and rsp.data["value"] == "NAK":
            return rsp
        self.cdc.xmlread = False
        resData = bytearray()
        if not rsp.resp:
            ...
            while bytestoread > 0:
                try:
                    tmp = self.cdc.read(min(self.cdc.maxsize, bytestoread), timeout=10)
                except Exception as e:
                    self.error(f"USB read error: {str(e)}")
                    return response(resp=False, data=resData, error=str(e))
                size = len(tmp)
                if size == 0:
                    # Device might need more time, but we've hit the end of transmission
                    # Continue to wait for final response from device
                    break
                bytestoread -= size
                resData.extend(tmp)
                progbar.show_progress(prefix="Read", pos=total - bytestoread, total=total, display=display)
            ...
            info = self.xml.getlog(wd)
            rsp = self.xml.getresponse(wd)
            if isinstance(rsp, dict) and "value" in rsp:
                if rsp["value"] != "ACK":
                    self.error(f"Error:")
                    for line in info:
                        self.error(line)
                ...
                # For ACK responses
                if rsp["value"] == "ACK":
                    return response(resp=True, data=resData, error=rsp.get("value", "ACK"))
            else:
    ...
    def get_gpt(self, lun, gpt_num_part_entries, gpt_part_entry_size, gpt_part_entry_start_lba, start_sector=1):
        try:
            ...
            try:
                ...
                self.error(f"Failed to read GPT from LUN {lun}: {str(err2)}")
                return None, None

        if not resp.resp:
            self.warning(f"LUN {lun} read failed, may not be available")
            for line in resp.error:
                self.debug(line)
            return None, None
        data = resp.data
        if len(data) < 4:
            self.warning(f"LUN {lun} returned insufficient data")
            return None, None
    ...
    def configure(self, lvl):
        # Force UFS sector size to 4096 - UFS devices always use 4096 byte sectors
        if self.cfg.MemoryName.lower() == "ufs":
            self.cfg.SECTOR_SIZE_IN_BYTES = 4096
        elif self.cfg.SECTOR_SIZE_IN_BYTES == 0:
        ...
        if not rsp.resp:
            ...
            for line in rsp.error:
                if "Not support configure MemoryName eMMC" in line:
                    ...
                    if self.modules.edlauth():
                        ...
                        return False
                elif isinstance(rsp.data, dict) and "MaxPayloadSizeToTargetInBytes" in rsp.data:
                    try:
                        self.cfg.MemoryName = rsp.data["MemoryName"]
                        self.cfg.MaxPayloadSizeToTargetInBytes = int(rsp.data["MaxPayloadSizeToTargetInBytes"])
                        self.cfg.MaxPayloadSizeToTargetInBytesSupported = int(
            ...
            # Skip initial read test if memory type explicitly specified via --memory
            if self.args["--memory"] is not None:
                self.info(f"Memory type explicitly set to {self.args['--memory'].upper()}, skipping initial read test")
                rsp = response(resp=True, data=b"", error="")
            else:
                self.info("Trying to read first storage sector...")
                # Add retry logic for devices which may need time to initialize
                rsp = None
                max_retries = 3
                for retry in range(max_retries):
                    try:
                        rsp = self.cmd_read_buffer(0, 1, 1, False)
                        if rsp.resp or (not rsp.resp and "Failed" in str(rsp.error)):
                            break
                    except Exception as e:
