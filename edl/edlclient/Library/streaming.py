...
class Streaming(metaclass=LogBase):
    def __init__(self, cdc, sahara, loglevel=logging.INFO, memory_type=None):
        ...
        self.nandbase = 0
        self.memory_type = (memory_type.lower() if memory_type else None)
    ...
    def get_flash_config(self):
        ...
        """
        # Skip NAND register configuration for UFS devices
        if self.memory_type == "ufs":
            self.info("UFS device detected. Skipping NAND register configuration.")
            return
    ...
    def nand_post(self):
        # Skip NAND post-processing for UFS devices
        if self.memory_type == "ufs":
            return
    ...
    def nand_init(self, xflag=0):
        # Skip NAND initialization for UFS devices
        if self.memory_type == "ufs":
            self.info("UFS device detected. Skipping NAND initialization.")
            return
    ...
    def flash_read(self, block, page, sectors, cwsize=None):
        # Skip NAND flash read for UFS devices
        if self.memory_type == "ufs":
            self.warning("UFS device detected. Skipping NAND flash_read operation.")
    ...
    def read_partition_table(self):
        # UFS devices use GPT partitioning, not NAND partition table
        if self.memory_type == "ufs":
            self.info("UFS device detected. Partition table will be read via Firehose protocol.")
            return b""
    ...
    def get_partitions(self, filename=""):
        ...
        # UFS devices use Firehose GPT protocol instead of NAND partition table
        if self.memory_type == "ufs" and filename == "":
            self.info("UFS device detected. Use Firehose protocol to read GPT partitions.")
            return partitions
    ...
    def connect(self, mode=1):
        ...
        if resp[0]:
            ...
            elif self.streaming_mode == self.Patched and self.memory_type == "ufs":
                self.info("Chipset: %s" % self.settings.chipname)
                self.info("UFS Storage Device detected (Sector size: %d bytes)" % sectorsize)
