...
class streaming_client(metaclass=LogBase):
    def __init__(self, arguments, cdc, sahara, loglevel, printer):
        ...
        # Extract memory type from arguments if provided
        memory_type = arguments.get("--memory", None)
        self.streaming = Streaming(cdc, sahara, loglevel, memory_type=memory_type)
        ...
        if self.streaming.connect(mode):
            ...
            if cmd == "gpt":
                ...
                if partitions:
                    ...
                    if self.streaming.memory_type == "ufs":
                        self.printer("UFS device detected. To read GPT, use Firehose protocol via:")
                        self.printer("  edl printgpt --memory=ufs")
                        self.printer("(GPT reading is not supported via streaming protocol on UFS devices)")
                    else:
                        self.printer("No partitions found.")
            elif cmd == "r":
